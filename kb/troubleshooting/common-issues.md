...
| Symptom | Cause | Fix |
|---|---|---|
| Plugin refuses to mount, log names the file-open seam | The harness no longer exposes `remote.session.openWorkspacePath` | Check the harness version against the peer range; the plugin fails closed on purpose rather than swallowing gestures silently |
