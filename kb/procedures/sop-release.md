...
created: "2026-09-04"
description: "Prepare a dsh-file-preview release and publish it with the single local release command."
---
...
6. Run `npm run release -- X.Y.Z`. It stops at the first failure and logs to `${TMPDIR:-/tmp}/dsh-file-preview-release-X.Y.Z/`:
...
| 2. Version and notes | `verify-version-sync.mjs --tag vX.Y.Z` (manifest, newest CHANGELOG heading), a dated `## [X.Y.Z]` heading, `HEAD` is `chore: release vX.Y.Z` |
...
| 5. Build and smoke | `npm pack`; the tarball must carry all four `lib/` entry points, the declarations, `cordis.patch.yml`, `LICENSE` and `NOTICE` and no sources, tests, KB or scripts; `lib/index.js` and `lib/host.js` are imported once from the unpacked tarball |
| 6. Tag and push | SSH-signed tag `vX.Y.Z` (`Release vX.Y.Z`) on `HEAD`, push `main`, then `git push origin refs/tags/vX.Y.Z` |
| 7. Watch publish | `gh run watch` on the tag's `publish.yml` run, then `npm view @softspark/dsh-file-preview@X.Y.Z` and the GitHub Release |

7. Execute [the post-release SOP](sop-post-release-testing.md) against the exact npm artifact and record browser, authorization, signatures, and provenance results.

**Why the Linux run applies here.** The host authorizes paths through realpath and symlink checks, and macOS hands out `/var` paths that resolve to `/private/var`. `publish.yml` also builds the artefact on Linux with native tools (rolldown, lightningcss). Either could fail on Linux only, after the tag. The old CI also ran on Windows; nothing local replaces that, so Windows path handling is covered only by review.
