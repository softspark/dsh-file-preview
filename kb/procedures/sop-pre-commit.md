...
Catch problems before the commit exists. No CI runs on pushes or pull requests; the only other place this gate runs is `pnpm run release`, where a failure blocks the tag.
...
5. `pnpm run build`: compile both host and browser declarations, build both artifacts, and verify every export target exists.
