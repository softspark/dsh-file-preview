#!/usr/bin/env bash
# Release @softspark/dsh-file-preview. The only supported way to create a release tag.
#
...
PKG="@softspark/dsh-file-preview"
GH_REPO="softspark/dsh-file-preview"
BRANCH="main"
LINUX_IMAGE="node:24"
...
# its own `|| return 1`.
pack_smoke() {
  local work tarball f target="node_modules/.release-smoke/dsh-file-preview"
  ...
  for f in lib/index.js lib/host.js lib/invariant.js lib/client.js lib/types/index.d.ts \
  ...
  ! grep -Eq '^(src|tests|kb|scripts)/' "$work/entries.txt" || { echo "tarball ships sources, tests, KB or scripts"; return 1; }
  ...
  node --input-type=module -e "
    ...
    const host = await import('./$target/lib/host.js');
    if (typeof entry.apply !== 'function') throw new Error('lib/index.js exports no apply()');
    if (typeof host.FilePreviewGateway !== 'function') throw new Error('lib/host.js exports no FilePreviewGateway');
    console.log('import ok: apply, FilePreviewGateway');
  " || return 1
  rm -rf "$target" "$work"
}

# Step 4 applies: the host authorizes paths through realpath and symlink checks
# (macOS hands out /var paths that resolve to /private/var), and publish.yml
# builds the artefact on Linux with native tools (rolldown, lightningcss). A
# Linux-only build or path failure would otherwise surface after the tag. The
# old CI also ran on Windows; nothing local replaces that.
#
...
if [ "$GATES_ONLY" = 1 ]; then
  LOG="$TMP_ROOT/dsh-file-preview-gates.log"; : >"$LOG"
  SARIF="$TMP_ROOT/dsh-file-preview-audit.sarif"
  step "Gates (log: $LOG)"
  gates
  exit 0
fi
...
[ -n "$VERSION" ] || usage
TAG="v$VERSION"
OUT="$TMP_ROOT/dsh-file-preview-release-$VERSION"
mkdir -p "$OUT"
...
# package.json, the newest CHANGELOG heading and the tag.
